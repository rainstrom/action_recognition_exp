...
flow_frames = '/scratch/xiaoyang/UCF101_opt_flows_org2/'
RGB_frames = '/scratch/xiaoyang/UCF101_frames_org2/'
test_frames = 16
train_frames = 16
test_buffer = 1 # 3
train_buffer = 3 # 24
...
class sequenceGeneratorVideo(object):
  ...
  def __call__(self):
    ...
    for i in idx_list:
      ...
      rand_frame = int(random.random()*(self.video_dict[key]['num_frames']-self.clip_length))
      frames = []
...
class videoRead(caffe.Layer):
  ...
  def setup(self, bottom, top):
    ...
    for ix, line in enumerate(f_lines):
      ...
      l = int(line.split(' ')[1]) 
      frames = glob.glob('%s%s/*.jpg' %(self.path_to_images, video_path))
      num_frames = len(frames)
      video_dict[video] = {}
      video_dict[video]['frames'] = os.path.join(self.path_to_images, video_path, '%04d.jpg')
      video_dict[video]['reshape'] = (240,320)
      video_dict[video]['crop'] = (224, 224)
